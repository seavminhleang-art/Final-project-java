...
public class ExamServiceImpl implements ExamService {
    ...
    public ExamServiceImpl(AttemptDao attemptDao, repository.QuizDao quizDao, AnnouncementService announcementService) {
    ...
    @Override
    public boolean canReview(Long attemptId, Long studentId) {
        Attempt attempt = get(attemptId);
        if (!attempt.getStudentId().equals(studentId)) return false;
        if (attempt.getStatus() == AttemptStatus.IN_PROGRESS) return false;
        return quizDao.findById(attempt.getQuizId())
                .map(Quiz::isReviewAllowed)
                .orElse(false);
    }
}
